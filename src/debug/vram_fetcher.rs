...
impl VRAMFetcher {
    pub(crate) fn render_all_background_tiles(
        &self,
        _: &dyn MapsMemory,
    ) -> ImageBuffer<Rgba<u8>, Vec<u8>> {
        ImageBuffer::from_fn(64, 384, |_, _| Rgba([255u8; 4]))
    }

    pub(crate) fn render_background_tilemap(
        &self,
        memory: &dyn MapsMemory,
    ) -> ImageBuffer<Rgba<u8>, Vec<u8>> {
        ...
        let bg_tiles_address: u16 = if (lcd_control_register >> 4) & 1 != 0 {
            0x8800
        } else {
            0x8000
        };
        for sprite_y in 0u16..16 {
            for sprite_x in 0u16..16 {
                ...
                for pixel_y in 0u16..8 {
                    let address =
                        bg_tiles_address + pixel_y * 0x2 + ((sprite_x + (sprite_y * 16)) * 0x10);
